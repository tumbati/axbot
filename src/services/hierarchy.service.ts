...
async function updateHierarchyLevel(account: string, level: string) {
  const result = await HierarchyModel.updateOne({ account }, { $set: { level } });
  return result.modifiedCount > 0;
}

async function sendMainMenu(account: string) {
  try {
...
async function filtrateChildren(filtration: ChildFiltration) {
  try {
...
async function filtrateInHierarchy(account: string, value: string) {
  try {
